...
INNER JOIN (
	...
	FROM (
		SELECT Y.organisationId
			,SUM(Y.entityCountAccess) AS entityCountAccess
			,Y.entityIdF
		FROM (
			SELECT D.organisationId
				,F.entityIdF
				,D.geoNodeId
				,CAST(F.entityIdF AS INT) AS entityCountAccess
			FROM [dwh].CT_OrgToGeoNode AS D
			INNER JOIN [dwh].CT_GeoNode_Closure AS E WITH (NOLOCK)
				ON D.geoNodeId = E.parent
			INNER JOIN (
				SELECT *
				FROM (
					SELECT COUNT(entityId) AS entityIdF
						,geoNodeId
					FROM [dwh].ET_Entity
				...
				ON D.geoNodeId = F.geoNodeId
			) AS Y
		GROUP BY Y.organisationId,Y.entityIdF
		) AS W
	) AS G
