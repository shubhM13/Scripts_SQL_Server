...
(
		SELECT A.[interactionId]
			...
			,ISNULL(G.groupCodes, '') AS criteriaGroupCodes
		...
		LEFT JOIN [dwh].[AT_Section_Txt] AS D ON D.sectionId = F.sectionId
			AND D.language = 'E'
		LEFT JOIN [dm].[dim_criteria_group_flat] AS G ON B.observationCriteriaId = G.criteriaId
...
-- VIew top 1000
SELECT TOP(1000) *
FROM [dm].[view_fact_assessment_analysis]

DROP TABLE [dm].[fact_assessment_analysis]

SELECT *
INTO [dm].[fact_assessment_analysis]
FROM dm.view_fact_assessment_analysis;

--ALTER TABLE [dm].[fact_assessment_analysis] ADD CONSTRAINT dimfactAss_pk PRIMARY KEY (interactionId, observationId);

SELECT COUNT(*)
FROM [dm].[view_fact_assessment_analysis]

SELECT COUNT(*)
FROM [dm].[dim_observation]

SELECT COUNT(*)
FROM [dm].[fact_assessment_analysis]

select distinct criteriaGroupCOdes from [dm].[fact_assessment_analysis]
 

select * from dm.view_fact_assessment_analysis
where interactionId = '501162DFC0E36E47'
AND observationId = '501162DFC1561A87'
