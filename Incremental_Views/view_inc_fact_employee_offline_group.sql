...
*/
/****** Object:  View [dm].[view_inc_fact_employee_offline_group]    Script Date: 22/07/2021 10:50:50 AM ******/
SET ANSI_NULLS ON
GO
 ...
 *************************a******************/
--drop view dm.view_inc_fact_employee_offline_group				
CREATE VIEW [dm].[view_inc_fact_employee_offline_group]
AS
(
		SELECT DISTINCT (A.employeeId) AS employeeIdOffline
			...
			FROM (
				...
				FROM (
					SELECT DISTINCT (A.organisationId) AS organisationIdUser
						,W.*
					FROM [dwh].[CT_Employee] AS A
					INNER JOIN [dwh].[CT_OrgToGeoNode] AS B WITH (NOLOCK) ON A.organisationId = B.organisationId
					INNER JOIN [dwh].[CT_GeoNode_Closure] AS C WITH (NOLOCK) ON B.geoNodeId = C.parent
					LEFT JOIN (
						SELECT *
						FROM (
							...
							INNER JOIN [dm].[dim_group_to_entity] AS T ON S.groupId = T.groupId
							INNER JOIN [dwh].[ET_Group] AS U ON S.groupId = U.groupId
								AND U.type = 'MOBILE_OFFLINE'
							INNER JOIN [dwh].[ET_Entity] AS V ON S.entityId = V.entityId
								AND V.STATUS <> 'DELETED'
							) AS Q
						) AS W ON C.child = W.geoNodeId
					) AS P
				GROUP BY P.organisationIdUser
					,P.groupId
					,P.entityId
				) AS M
			) AS T ON A.organisationId = T.organisationIdUser
		AND A.[auditInfo.modifiedOn] >= (select waterMarkVal from [AUDIT].[WaterMark] where schemaname = 'CT' and sqltablename = 'Employee')
		INNER JOIN [dwh].[CT_UserSetting] AS D ON T.groupId = D.offlineMobileGroup
			AND A.userName = D.userName
		INNER JOIN [dwh].[ET_Group] AS F ON D.offlineMobileGroup = F.groupId
		);
GO