...
/*******************************************
 ...
 Created On : 5th August, 2021
 PURPOSE    : Nescafe Event Analysis
 *******************************************/
--drop view [dm].[view_dim_nsc_attended_persons]
CREATE VIEW [dm].[view_dim_nsc_attended_persons]
AS
(
		...
		WHERE A.personId IN (select distinct personId from [dm].[bridge_nsc_event_to_attended_persons_entities])
		);
GO

drop table [dm].[dim_nsc_attended_persons];

select * 
into [dm].[dim_nsc_attended_persons]
from [dm].[view_dim_nsc_attended_persons];

ALTER TABLE [dm].[dim_nsc_attended_persons] ADD CONSTRAINT dimnscAttPerson_pk PRIMARY KEY (personId);