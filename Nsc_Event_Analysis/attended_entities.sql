/****** Object:  View [dm].[view_nsc_entity_master]    Script Date: 31/05/2021 1:36:07 AM ******/
SET ANSI_NULLS ON
GO
...
/*******************************************
 ...
 PURPOSE    : Entities Attending an event (Nescafe)
 *******************************************/
--DROP VIEW [dm].[view_dim_nsc_attended_entities];
CREATE VIEW [dm].[view_dim_nsc_attended_entities]
AS
(
		SELECT DISTINCT A.entityId AS entityId
			...
			,ISNULL(D.sub_cluster_name, '') AS subClusterName
			,ISNULL(D.cluster_name, '') AS clusterName
			,ISNULL(D.country_name, '') AS countryName
		...
		LEFT OUTER JOIN dwh.CT_ListOption_Txt AS N WITH (NOLOCK) ON B.coreAssessmentStatus = N.itemCode
			...
			AND N.LANGUAGE = 'E'
		WHERE A.entityId IN (select distinct entityId from [dm].[bridge_nsc_event_to_attended_persons_entities])
		);
GO


drop table [dm].[dim_nsc_attended_entities];

SELECT
  *
INTO [dm].[dim_nsc_attended_entities]
FROM [dm].[view_dim_nsc_attended_entities];


ALTER TABLE [dm].[dim_nsc_attended_entities]
ADD CONSTRAINT nscAttEntity_pk PRIMARY KEY (entityId);
