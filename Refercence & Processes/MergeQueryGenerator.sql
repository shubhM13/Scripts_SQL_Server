...
BEGIN
	...
	SELECT A.column_name
		,CASE 
			...
			ELSE 0 
			END AS IsIdentity
		,COLUMNPROPERTY(object_id(@TgtSchemaName+'.'+@TgtTableName), A.COLUMN_NAME, 'IsComputed') AS IsComputed
		,A.DATA_TYPE
	FROM information_schema.columns AS A
	INNER JOIN INFORMATION_SCHEMA.TABLE_CONSTRAINTS AS B
	ON A.TABLE_NAME = B.TABLE_NAME
    ...
    AND B.CONSTRAINT_SCHEMA = C.CONSTRAINT_SCHEMA
    AND B.CONSTRAINT_NAME = C.CONSTRAINT_NAME
	WHERE A.TABLE_NAME = @TgtTableName
...
EXEC [dm].[GenerateMergeSQL] 'dm', 'fact_employee_mobile_sync_analysis'
